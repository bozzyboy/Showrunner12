...
import { AssetType, Character, Location, Prop, Project } from '../../types';
    ...
    const galleryData = useMemo(() => {
        ...
        if (project.studio && project.studio.shotsByScene) {
            ...
            items.forEach((item: any) => {
                const subItems = isEpisodic ? item.episodes : item.acts;
                subItems.forEach((sub: any) => {
                    sub.scenes.forEach((sc: any) => {
                        const code = isEpisodic 
                            ? `S${item.seasonNumber}E${sub.episodeNumber}` 
                            : `P${item.partNumber}A${sub.actNumber}`;
                        sceneMap.set(sc.id, `${code} Sc ${sc.sceneNumber}`);
                    });
                });
            });
