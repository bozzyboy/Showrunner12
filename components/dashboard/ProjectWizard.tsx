...
const ProjectWizard: React.FC<ProjectWizardProps> = ({ onClose }) => {
  ...
  const handleFinish = () => {
    const projectFormat: ProjectFormat = {
      ...
      episodeCount: formatType === 'EPISODIC' ? episodeCount : 3, // 3 acts for film
      duration: duration,
        ...
        return (
          <div>
            ...
            <div className="space-y-4">
              ...
              <div>
               ...
               <div>
                  ...
                  <div 
                    ...
                    <p className="text-xs text-muted">Scripts, notes, and other text-based files.</p>
                    <input ref={fileInputRef} type="file" multiple className="hidden" onChange={handleFileSelect} accept=".txt,.md,text/plain,text/markdown" />
                  </div>
              ...
              <div>
                <label className="block text-sm font-medium text-primary-text">Format</label>
                <div className="mt-2 grid grid-cols-2 gap-4">
                  <div onClick={() => { setFormatType('EPISODIC'); setDuration('22'); }} className={`cursor-pointer p-4 rounded-lg border-2 ${formatType === 'EPISODIC' ? 'border-accent bg-accent/10' : 'border-subtle bg-panel hover:border-muted'}`}>
                    <h4 className="font-bold text-primary">Episodic Series</h4>
                    <p className="text-xs text-muted">A series with multiple episodes and seasons.</p>
                  </div>
                  <div onClick={() => { setFormatType('SINGLE_STORY'); setDuration('90'); }} className={`cursor-pointer p-4 rounded-lg border-2 ${formatType === 'SINGLE_STORY' ? 'border-accent bg-accent/10' : 'border-subtle bg-panel hover:border-muted'}`}>
                    <h4 className="font-bold text-primary">Single Story</h4>
                    <p className="text-xs text-muted">A feature-length story, with sequel potential.</p>
                  </div>
                </div>
              </div>
              {formatType === 'EPISODIC' ? (
        ...
        return (
          <div>
            ...
            <div className="space-y-3 text-sm bg-panel p-4 rounded-lg border border-subtle">
                ...
                <p><strong className="text-muted">Format:</strong> {formatType === 'EPISODIC' ? `${seasonCount} Season(s), ${episodeCount} episodes per season` : 'Single Story'}</p>
